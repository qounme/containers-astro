...
const validatePhoneNumber = (data: { phoneNumbers: [string, string, string] }) =>
  PHONE_NUMBER_REGEX.test(`${data.phoneNumbers[0]}${data.phoneNumbers[1]}${data.phoneNumbers[2]}`)
...
export const singlePageApplicationFormSchema = z
  .object({
    ...
    phoneNumbers: z.tuple([z.string(), z.string(), z.string()]),
  ...
  .refine(validatePhoneNumber, { message: '携帯電話番号を入力してください', path: ['phoneNumbers'] })
